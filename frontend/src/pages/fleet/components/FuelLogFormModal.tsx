    ...
    <BaseModal
      ...
      footer={
        <div className="modal-footer">
          ...
          <button type="submit" form={FORM_ID} className="btn-primary" disabled={mutation.isPending || isUploading}>
          ...
          <div className="form-grid" style={{ gridTemplateColumns: '1fr' }}>
            ...
            <div className="input-group">
              ...
              <select {...register('driverId')}>
                ...
                {(driversData ?? []).filter((d: any) => d.status === 'Active').map((d: any) => (
                  <option key={d.id} value={d.id}>{d.name} - {d.assignedVehicle || d.cnhNumber}</option>
                ))}
              </select>
            </div>
